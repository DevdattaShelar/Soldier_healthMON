...
const firebaseConfig = {
  apiKey: "AIzaSyBRXihE_3t57xVp2qWiMlmwDJymAjEYtSI",
  authDomain: "soldier-d809f.firebaseapp.com",
  databaseURL: "https://soldier-d809f-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId: "soldier-d809f",
  storageBucket: "soldier-d809f.firebasestorage.app",
  messagingSenderId: "730167176900",
  appId: "1:730167176900:web:0605306c207ae9b16c6391",
  measurementId: "G-P8SYQLD5TE"
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Firebase Integration (Only for UN-001) ---
    const unit1Ref = ref(database, 'units/UN-001');
    onValue(unit1Ref, (snapshot) => {
        ...
        if (data) {
            ...
            if (index !== -1) {
                ...
                if (unit.vitals) {
                    ...
                    unit.status = 'unknown';
                    unit.vitals = { hr: 0, spo2: 0, temp: 0, gas: 'unknown', movement: 'unknown' };
                }
                state.units[index] = unit;
            }
        }

        // Auto-update dashboard if logged in
        if (state.isAuthenticated) {
            if (state.activeView === 'dashboard-view' || state.activeView === 'units-view' || state.activeView === 'alerts-view') {
                renderDashboard();
            } else if (state.activeView === 'unit-detail-view' && state.selectedUnitId === 'UN-001') {
                const activeU = state.units.find(u => u.id === 'UN-001');
    ...
    function calculateAlerts() {
        ...
        state.units.forEach(unit => {
            if (unit.status === 'critical') {
                ...
                if (unit.vitals.gas !== 'Safe air') showNotification(`${unit.name}: Harmful Gas Detected!`, 'critical');
